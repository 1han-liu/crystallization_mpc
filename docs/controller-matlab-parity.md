# Controller MATLAB to Python parity
...
This conversion owns the numerical behavior and persistent algorithm state of
...
|---|---|---|
| `source_codes/controller_for_gui.m` | convert + platform | `translated/matlab_controller.py`; lifecycle and finite replay tests; communication/GUI parts are platform-owned |
...
| `control/calc_G.m`, `calc_c_meta.m`, `calc_c_sat.m`, `calc_dc_sat_dT.m`, `calc_relative_sigma.m`, `calc_sigma.m` | convert | `translated/thermodynamics.py`; algebra oracle |
| `control/calc_K_I_target.m`, `calc_K_P_target.m` | convert | `translated/control.py`; mode oracle |
| `control/calc_Q.m`, `calc_R.m`, `measurement_function.m`, `measurement_matrices.m`, `state_transition_matrices.m`, `state_transition_matrices_model.m` | convert | `translated/ekf.py`; matrix oracle |
| `control/construct_EKF.m`, `create_EKF_general.m`, `smooth_EKF.m`, `smooth_EKF_general.m` | convert | explicit state/covariance EKF in `translated/ekf.py`; trajectory oracle |
| `control/calc_T_j.m`, `calc_T_j_set.m`, `calc_T_j_set_.m`, `calc_T_with_zero_dT_dt.m`, `calc_dT_dt_model.m`, `calc_dT_dt_set.m`, `calc_dT_j_dt.m`, `calc_e_dT_dt.m`, `calc_e_target.m`, `objective_function.m`, `update_T_j.m` | convert | `translated/control.py`; PI/MPC and trajectory oracles |
| `control/calc_dT_dt.m`, `calc_dc_dt.m`, `calc_t_lag_perc.m`, `calc_t_lag_perc2.m`, `clip.m` | convert | `translated/control.py`; boundary tests |
| `control/state_transition_ODE.m`, `state_transition_ODE_T.m`, `state_transition_function.m`, `state_transition_function_T.m` | convert | `translated/dynamics.py`; RK45 oracle |
| `control/calc_size_from_volume.m`, `calc_surface_area.m`, `calc_volume.m` | canonical duplicate | `translated/mass_balance.py`; identical to mass-balance copies |
| `control/read_value.m`, `safe_read_value.m` | platform | `apps/controller/process.py`; not called by numerical code |
| `mass_balance/calc_next_crystallization_mass_balance.m`, `create_size_list.m` | convert | `translated/mass_balance.py`; deterministic fixture arrays |
| `mass_balance/calc_size_from_volume.m`, `calc_surface_area.m`, `calc_volume.m` | convert/canonical | `translated/mass_balance.py`; algebra oracle |
| `utils/fill_target_list.m`, `initialize_lists.m` | convert | controller state/history helpers and mode tests |
| `utils/initialize_G.m` | split | simulated branch in translated controller; image/Gsensor initialization is platform-owned |
...
| `snipptets_controller/calculate_target.m`, `calculate_lag_time.m`, `calculate_T_j_set.m`, `control_target.m` | convert | ordered operations in `MatlabController.step`; replay oracle |
| `snipptets_controller/execute_growth_parameters_adaption.m`, `record_growth_parameters.m`, `refresh_adaptive.m`, `refresh_controller.m` | convert | adaptation/EKF state in translated modules; state tests |
| `snipptets_controller/initialize_controller.m`, `initialize_controller_for_gui.m`, `record_controller_time.m` | convert | `MatlabController.start/step`; lifecycle tests |
...
The translated adapter remains opt-in. The full verification command is
